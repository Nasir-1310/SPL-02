# This file contains the main class of DroidInspect
# It can be used after AVD was started, app was installed, and adb had been set up properly
# By configuring and creating a DroidInspect instance,
# DroidInspect will start interacting with Android in AVD like a human
import logging
import os
import sys
...
class DroidInspect(object):
    """
    The main class of DroidInspect
    """
    ...
    def __init__(self,
                 ...
                 replay_output=None):
        """
        initiate DroidInspect with configurations
        :return:
        """
        logging.basicConfig(level=logging.DEBUG if debug_mode else logging.INFO)

        self.logger = logging.getLogger('DroidInspect')
        DroidInspect.instance = self

        self.output_dir = output_dir
        if output_dir is not None:
            if not os.path.isdir(output_dir):
                os.makedirs(output_dir)
            html_index_path = pkg_resources.resource_filename("DroidInspect", "resources/index.html")
            stylesheets_path = pkg_resources.resource_filename("DroidInspect", "resources/stylesheets")
    ...
    @staticmethod
    def get_instance():
        if DroidInspect.instance is None:
            print("Error: DroidInspect is not initiated!")
            sys.exit(-1)
        return DroidInspect.instance
    ...
    def start(self):
        ...
        if not self.enabled:
            return
        self.logger.info("Starting DroidInspect")
        try:
            if self.timeout > 0:
        ...
        self.stop()
        self.logger.info("DroidInspect Stopped")
...
class DroidInspectException(Exception):
    pass
