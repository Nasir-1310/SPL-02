...
class ContactAppEnv(StaticAppEnv):
    ...
    def __init__(self, name='Lynn', phone="1234567890", email="DoridInspect@honeynet.com", env_dict=None):
...
class DummyFilesEnv(StaticAppEnv):
    ...
    def __init__(self, dummy_files_dir=None):
        ...
        if dummy_files_dir is None:
            ...
            dummy_files_dir = pkg_resources.resource_filename("DoridInspect", "resources/dummy_documents")
...
class AppEnvManager(object):
    ...
    def deploy(self):
        ...
        if self.device.output_dir is not None:
            out_file = open(os.path.join(self.device.output_dir, "DoridInspect_env.json"), "w")
            self.dump(out_file)
            out_file.close()
            self.logger.debug("Environment settings saved to DoridInspect_env.json")
