# DoridInspectScript
# This file contains the definition of DoridInspectScript
# DoridInspectScript is a domain-specific language, which defines how DoridInspect interacts with target app
import logging
import random
import re
...
DoridInspectOperation_VAL = 'DoridInspectOperation'
DoridInspectAction_VAL = 'DoridInspectAction'
ScriptEvent_VAL = 'ScriptEvent'


class DoridInspectScript(object):
    """
    DoridInspectScript is a DSL for customizing input for certain states
    """
    script_grammar = {
        'views': {
        ...
        'operations': {
            OPERATION_ID: DoridInspectOperation_VAL
        },
        'main': {
            STATE_ID: DoridInspectAction_VAL
        }
    }
    ...
    def parse_operations(self):
        ...
        for operation_id in script_value:
            ...
            operation = DoridInspectOperation(operation_id, event_list, self)
    ...
    def parse_main(self):
        ...
        for state_id in script_value:
            ...
            if isinstance(action[0], str):
                self.main[state_selector] = RoundRobinDoridInspectAction(action, self, state_key_tag)
            else:
                self.main[state_selector] = ProbabilisticDoridInspectAction(action, self, state_key_tag)
    ...
    def check_grammar_prob_operation_is_valid(value, key_tag):
        ...
        for key in ['op_id', 'prob']:
            DoridInspectScript.check_grammar_has_key(value.keys(), key, key_tag)
...
class ViewSelector(object):
    ...
    def parse(self):
        DoridInspectScript.check_grammar_type(self.selector_dict, self.selector_grammar, self.tag)
        for selector_key in self.selector_dict:
            DoridInspectScript.check_grammar_key_is_valid(selector_key, self.selector_grammar, self.tag)
            ...
            DoridInspectScript.check_grammar_type(selector_value, grammar_value, key_tag)
            if selector_key == 'text':
                ...
                for out_coordinate in selector_value:
                    DoridInspectScript.check_grammar_is_coordinate(out_coordinate)
                    self.out_coordinates.append(out_coordinate)
            elif selector_key == 'in_coordinates':
                for in_coordinate in selector_value:
                    DoridInspectScript.check_grammar_is_coordinate(in_coordinate)
        ...
        for selector_key in self.selector_dict:
            ...
            if selector_key == 'activity':
                ...
                for view_id in selector_value:
                    DoridInspectScript.check_grammar_key_is_valid(view_id, self.script.views, key_tag)
...
class DoridInspectAction:
    """
    an action is what DoridInspect would do on device at specific states
    """
    @abstractmethod
...
class RoundRobinDoridInspectAction(DoridInspectAction):
    """
...
class ProbabilisticDoridInspectAction(DoridInspectAction):
    """
...
class DoridInspectOperation(object):
    """
    an operation is what DoridInspect do to target device
    ...
    def parse(self):
        DoridInspectScript.check_grammar_is_list(self.event_list)
        for event_dict in self.event_list:
            if 'target_view' in event_dict:
                target_view_id = event_dict['target_view']
                DoridInspectScript.check_grammar_key_is_valid(target_view_id, self.script.views, self.tag)
...
class ScriptEvent:
    """
    an event defined in DoridInspectScript
...
class ScriptSyntaxError(RuntimeError):
    """
    syntax error of DoridInspectScript
    """
    pass
...
if __name__ == '__main__':
    ...
    test_script = DoridInspectScript(json.load(open("script_samples/probabilistic_script.json", "r")))
