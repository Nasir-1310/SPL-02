...
class UtgNaiveSearchPolicy(UtgBasedInputPolicy):
    ...
    def generate_event_based_on_utg(self):
        ...
        if self.device.is_foreground(self.app):
            ...
            # If we have tried too many times but the app is still not started, stop DoridInspect
    ...
    def select_a_view(self, state):
        """
        select a view in the view list of given state, let DoridInspect touch it
        ...
        # DoridInspect stuck on current state, return None
...
class UtgReplayPolicy(InputPolicy):
    """
    Replay DoridInspect output generated by UTG policy
    """
