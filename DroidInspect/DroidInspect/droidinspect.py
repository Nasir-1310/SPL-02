# This file contains the main class of doridinspect
# It can be used after AVD was started, app was installed, and adb had been set up properly
# By configuring and creating a doridinspect instance,
# doridinspect will start interacting with Android in AVD like a human
import logging
import os
import sys
...
class doridinspect(object):
    """
    The main class of doridinspect
    """
    ...
    def __init__(self,
                 ...
                 replay_output=None):
        """
        initiate doridinspect with configurations
        :return:
        """
        logging.basicConfig(level=logging.DEBUG if debug_mode else logging.INFO)

        self.logger = logging.getLogger('doridinspect')
        doridinspect.instance = self

        self.output_dir = output_dir
        if output_dir is not None:
            if not os.path.isdir(output_dir):
                os.makedirs(output_dir)
            html_index_path = pkg_resources.resource_filename("doridinspect", "resources/index.html")
            stylesheets_path = pkg_resources.resource_filename("doridinspect", "resources/stylesheets")
    ...
    @staticmethod
    def get_instance():
        if doridinspect.instance is None:
            print("Error: doridinspect is not initiated!")
            sys.exit(-1)
        return doridinspect.instance
    ...
    def start(self):
        ...
        if not self.enabled:
            return
        self.logger.info("Starting doridinspect")
        try:
            if self.timeout > 0:
        ...
        self.stop()
        self.logger.info("doridinspect Stopped")
...
class doridinspectException(Exception):
    pass
